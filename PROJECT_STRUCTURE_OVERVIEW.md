...
- A batch ingestion pipeline (`main.py`) that discovers from `rss`, `sitemap`, and `listing` sources, filters relevance, extracts content, and stores documents.
- A Streamlit operations UI (`dashboard.py`) for browsing docs, editing config, running pipeline jobs, and completing manual AI analysis.
...
| `modules/ingest.py` | Multi-source ingestion orchestration | Source configs + discovery candidates | Stored `documents` rows + stats | Calls discovery/fetch/filter/store pipeline |
...
| `modules/filter.py` | Tiered relevance scoring/filtering | Candidate title/summary + keyword/context lists | `FilterResult` | No persistent writes |
...
- Local test mode: `python main.py --test`
...
flowchart TD
    ...
    E --> H[modules.filter.check_relevance]
